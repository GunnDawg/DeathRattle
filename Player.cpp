...
Player::Player()
{
	m_playerTexture.Load(Game::Renderer, "Assets/Graphics/knight.png");
...
void Player::update()
{
	...
	{
		m_playerRect.x += static_cast<int>(m_moveSpeed * Game::deltaTime);
		m_cropRect.y = 196;
	}
	...
	{
		m_playerRect.y += static_cast<int>(m_moveSpeed * Game::deltaTime);
		m_cropRect.y = 128;
	}
...
void Player::draw()
{
	SDL_RenderCopy(Game::Renderer, m_playerTexture.m_Texture, &m_cropRect, &m_playerRect);
}